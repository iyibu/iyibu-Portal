(function ($) {
	var defaults = {
		butonlar: {},
		simge: 'hakkinda',
		icerik: '<a href="http://www.iyibu.org"><br><br><center>iyibu!Portal Kutucuk v3</center></a>',
		position: 'center',
		closeKey: true,
		closeOverlay: true,
		useOverlay: true,
	...
	methods = {
		hide: function (options, callback, self) {
			if (methods.isActive()) {
				$('#iyibuKutu-wrapper').stop(true, true);
				var $f = $('#iyibuKutu-wrapper'),
				pos = $f.css('position'),
				isFixed = (pos === 'fixed'),
				yminpos = 0;
				...
				(opts.duration || opts.duration), opts.easingOut, function () {
						$('#iyibuKutu-overlay').fadeOut('fast')
					$f.remove();
		...
		show: function (options, callback, self) {
			if (methods.isActive()) {
				...
				opts = $.extend({},
				defaults, options);
				$('<div id="iyibuKutu-wrapper"></div>').appendTo('body');
				...
				$f.css({
					'width': opts.genislik,
					'height': opts.yukseklik,
					'position': 'absolute',
					'top': '-9999px',
					'left': '-9999px'
				}).html('<div id="iyibuKutu-icon"></div>' + '<div id="iyibuKutu"></div>' + '<div id="iyibuKutu-buttons"></div>').find('#iyibuKutu-icon').addClass('icon-' + opts.simge).end().find('#iyibuKutu').html(opts.icerik).css({
					'height': (opts.yukseklik == 'auto') ? 'auto': $f.height() - 116,
					'width': $f.width() - 94
				}).end().find('#iyibuKutu-buttons').html((function () {
					var buttons = '';
					var i;
					for (i in opts.butonlar) {
						if (opts.butonlar.hasOwnProperty(i)) {
							buttons = buttons + '<a href="#" class="iyibuKutu-button ' + (opts.butonlar[i].danger ? 'iyibuKutu-button-danger': '') + '" id="iyibuKutu-button-' + i + '">' + opts.butonlar[i].text + '</a>'
						}
					}
					return buttons
				} ())).find('.iyibuKutu-button').bind('click', function () {
					var buttonId = $(this).attr('id').substring(17);
					if (typeof opts.butonlar[buttonId].onclick === 'function' && opts.butonlar[buttonId].onclick != false) {
						var scope = $('#iyibuKutu');
						opts.butonlar[buttonId].onclick.apply(scope)
					} else {
						methods.hide()
					}
					return false
				});
				var showKutucuk = function () {
					$f.show();
					...
					opts.duration, opts.easingIn, function () {
		...
		isActive: function () {
			return !! ($('#iyibuKutu-wrapper').length > 0)
		},
		blink: function () {
			$('#iyibuKutu-wrapper').fadeOut(100, function () {
				$(this).fadeIn(100)
			})
		},
		shake: function () {
			$('#iyibuKutu-wrapper').stop(true, true).animate({
				'left': '+=20px'
			},
			50, function () {
				$(this).animate({
	...
	helpers = {
		...
		enterKeyHandler: function (e) {
			if (e.keyCode === 13) {
				$('#iyibuKutu-buttons').children().eq(0).focus();
				helpers.unbindKeyHandler()
			}
		},
		tabKeyHandler: function (e) {
			if (e.keyCode === 9) {
				$('#iyibuKutu-wrapper').find(':input, .iyibuKutu-button').eq(0).focus();
				helpers.unbindKeyHandler();
				e.preventDefault()
			}
		},
	...
	$(document).ready(function () {
		$('body').append('<div id="iyibuKutu-overlay"></div>');
		$('#iyibuKutu-overlay').bind('click', function () {
	...
	$.iyibuKutu = function (method, options, callback) {
		var self = window;
		...
		if (methods[method]) {
			...
			$.info('"' + method + '": böyle bir method bulunamdý (iyibuKutucuk)')
		}
	}
} (jQuery));